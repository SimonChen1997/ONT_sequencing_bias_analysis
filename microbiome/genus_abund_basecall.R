...
mr.rarefied_basecall = rarefy_even_depth(merged_rumen_basecall, rngseed=7127, sample.size=min(sample_sums(merged_rumen_basecall)), replace=F)
...
rel_stat <- function(genus_join_lr_replicate) {
  relative_lr_csv <- genus_join_lr_replicate %>%
    group_by(location, taxon, sequencing_kit, basecall_mode) %>%
...
rel_compare_aus_archaea <- function(metadata, genus_join_lr_replicate) {
  print(by(genus_join_lr_replicate$rel_abund, genus_join_lr_replicate$sequencing_kit, shapiro.test))
  
  genus_join_lr_replicate_comparison <- genus_join_lr_replicate %>%
    filter(location=="Australia") %>%
    group_by(location, sequencing_kit, taxon) %>%
    t_test(rel_abund ~ basecall_mode, paired = F, var.equal =T) %>%
    add_significance %>%
    mutate(location = "Australia") %>%
    select(location, everything())
...
rel_compare_es_archaea <- function(metadata, genus_join_lr_replicate) {
  print(by(genus_join_lr_replicate$rel_abund, genus_join_lr_replicate$sequencing_kit, shapiro.test))
  
  genus_join_lr_replicate_comparison <- genus_join_lr_replicate %>%
    filter(location=="Spain") %>%
    group_by(location, sequencing_kit, taxon) %>%
    t_test(rel_abund ~ basecall_mode, paired = T, var.equal =T) %>%
    add_significance %>%
    mutate(location = "Spain") %>%
    select(location, everything())
...
###############################################################################################################
### bacteria analysis in basecall
k_name <- "Bacteria"
bacteria_mr <- kingdom_subset(mr.rarefied_basecall, k_name)
...
y.grob <- textGrob("Genus", 
                   gp=gpar(fontface="bold", col="black", fontsize=24), rot=90)

bacteria_taxon_seq_plot <- grid.arrange(arrangeGrob(plot_grid(bacteria_taxon_seq_aus_plot, bacteria_taxon_seq_es_plot, ncol=1, align = 'v'), left=y.grob))
...
### archaea analysis in basecall
k_name <- "Archaea"
archaea_mr <- kingdom_subset(mr.rarefied_basecall, k_name)
...
archaea_taxon_pool <- taxon_pool(archaea_relative_calculation, 8.5)
...
archaea_taxon_seq_plot <- grid.arrange(arrangeGrob(plot_grid(archaea_taxon_seq_aus_plot, archaea_taxon_seq_es_plot, ncol=1, align = 'v'), left=y.grob))
...
### eukaryota analysis in basecall
k_name <- "Eukaryota"
eukaryota_mr <- kingdom_subset(mr.rarefied_basecall, k_name)
...
eukaryota_taxon_pool <- taxon_pool(eukaryota_relative_calculation, 2.7)
...
eukaryota_taxon_seq_plot <- grid.arrange(arrangeGrob(plot_grid(eukaryota_taxon_seq_aus_plot, eukaryota_taxon_seq_es_plot, ncol=1, align = 'v'), left=y.grob))
