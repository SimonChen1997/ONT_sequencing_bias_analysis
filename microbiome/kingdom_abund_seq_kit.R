...
merged_rumen_seq_kit_sub = subset_samples(merged_rumen_seq_kit, (extraction_kit=="PowerFecal" | extraction_kit=="PowerSoil") & basecall_mode=="SUP")

mr.rarefied_seq_kit = rarefy_even_depth(merged_rumen_seq_kit_sub, rngseed=7127, sample.size=min(sample_sums(merged_rumen_seq_kit)), replace=F)
...
Kingdom_level <- mr.rarefied_seq_kit
...
genus_join_lr_replicate <- inner_join(genus_rel_abund_lr, genus_pool_lr, by="taxon") %>%
  ...
  group_by(location, sample_id, sequencing_kit, taxon) %>%
...
###############################################################################################################
by(genus_join_lr_replicate$rel_abund, genus_join_lr_replicate$sequencing_kit, shapiro.test)
...
kingdom_seq_aus <- genus_join_lr_replicate[genus_join_lr_replicate$location=="Australia",] %>%
...
kingdom_seq_es <- genus_join_lr_replicate[genus_join_lr_replicate$location=="Spain",] %>%
  ggplot(aes(x=fct_reorder(taxon, mean, .desc = F), y=rel_abund)) +
  geom_boxplot(aes(color=sequencing_kit, x=taxon), position = "dodge", lwd=1, width=0.4) +
  facet_wrap(~location) +
...
legend_seq <- get_legend(kingdom_seq_es + theme(legend.position="bottom",
                                                legend.title = element_text(size = 24)))

x.grob <- textGrob("Relative abundance (%)", 
                   gp=gpar(fontface="bold", col="black", fontsize=24), rot=0)

kingdom_seq <- grid.arrange(arrangeGrob(plot_grid(kingdom_seq_aus, kingdom_seq_es, nrow=1, align = 'h', labels="A", label_size=28, hjust = 2, rel_widths = c(1, 0.75)), bottom = x.grob))

kingdom_seq_legend <- plot_grid(kingdom_seq, legend_seq, ncol = 1, nrow = 2, labels="A", label_size=28, rel_heights = c(1, 0.1))
kingdom_seq_legend